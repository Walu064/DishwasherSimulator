...
{
    public class CapacitySensorClientWPF : IAsyncDisposable
    {
        HubConnection capacitySensorServer;

        public event EventHandler<int> ChangeLaundry;

        public CapacitySensorClientWPF()
        {
            capacitySensorServer = new HubConnectionBuilder()
                .WithUrl(@"http://localhost:36005/capacity_sensor")
            ...
            {
                ChangeLaundry?.Invoke(this, weight);
            });
        ...
        public async Task SimulateCapacitySensor(int numberOfDishes)
        {
            await capacitySensorServer.SendAsync("SimulateCapacitySensorTask", numberOfDishes);
        }
        public async Task SimulateCapacitySensorOut(int numberOfDishes)
        {
